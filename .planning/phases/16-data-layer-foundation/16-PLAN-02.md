...
# Plan 02: Favorites Repository Implementation
...
### Task 1: Create Favorites Repository

<read_first>
- /Volumes/DATA/Code/Vscode/wallhaven/src/repositories/download.repository.ts — best reference for collection-based repository pattern
- /Volumes/DATA/Code/Vscode/wallhaven/src/repositories/settings.repository.ts — simple repository pattern
- /Volumes/DATA/Code/Vscode/wallhaven/src/clients/electron.client.ts — storeGet/storeSet methods
- /Volumes/DATA/Code/Vscode/wallhaven/src/clients/constants.ts — STORAGE_KEYS
- /Volumes/DATA/Code/Vscode/wallhaven/src/types/favorite.ts — type definitions (created in Plan 01)
- /Volumes/DATA/Code/Vscode/wallhaven/src/shared/types/ipc.ts — IpcResponse interface
</read_first>

<action>
Create `src/repositories/favorites.repository.ts` with the following implementation:
...
export const favoritesRepository = {
  ...
  async setData(data: FavoritesData): Promise<IpcResponse<void>> {
    ...
    return electronClient.storeSet(STORAGE_KEYS.FAVORITES_DATA, updatedData)
  },
  ...
  async createCollection(name: string): Promise<IpcResponse<Collection>> {
    ...
    // 检查名称是否已存在
    if (result.data.collections.some(c => c.name === name)) {
    ...
    const updatedData: FavoritesData = {
      ...result.data,
      collections: [...result.data.collections, newCollection],
    }
  ...
  async renameCollection(id: string, name: string): Promise<IpcResponse<Collection>> {
    ...
    const collection = result.data.collections.find(c => c.id === id)
    if (!collection) {
      return createError(FavoritesErrorCodes.COLLECTION_NOT_FOUND, '收藏夹不存在')
    }

    // 检查新名称是否与其他收藏夹冲突
    if (result.data.collections.some(c => c.name === name && c.id !== id)) {
      return createError(FavoritesErrorCodes.COLLECTION_NAME_EXISTS, '收藏夹名称已存在')
    }
    ...
    const updatedData: FavoritesData = {
      ...result.data,
      collections: result.data.collections.map(c => (c.id === id ? updatedCollection : c)),
    }
  ...
  async deleteCollection(id: string): Promise<IpcResponse<void>> {
    ...
    const updatedData: FavoritesData = {
      ...result.data,
      collections: result.data.collections.filter(c => c.id !== id),
      favorites: result.data.favorites.filter(f => f.collectionId !== id),
    }
  ...
  async addFavorite(item: FavoriteItem): Promise<IpcResponse<FavoriteItem>> {
    ...
    // 检查收藏夹是否存在
    if (!result.data.collections.some(c => c.id === item.collectionId)) {
      return createError(FavoritesErrorCodes.COLLECTION_NOT_FOUND, '收藏夹不存在')
    }

    // 检查是否已收藏
    if (result.data.favorites.some(f => f.wallpaperId === item.wallpaperId && f.collectionId === item.collectionId)) {
      return createError(FavoritesErrorCodes.FAVORITE_ALREADY_EXISTS, '该壁纸已在此收藏夹中')
    }

    const updatedData: FavoritesData = {
      ...result.data,
      favorites: [...result.data.favorites, item],
    }
  ...
  async removeFavorite(wallpaperId: string, collectionId: string): Promise<IpcResponse<void>> {
    ...
    const index = result.data.favorites.findIndex(
    ...
    const updatedData: FavoritesData = {
      ...result.data,
      favorites: result.data.favorites.filter(
        f => !(f.wallpaperId === wallpaperId && f.collectionId === collectionId)
      ),
    }
  ...
  async moveFavorite(
    ...
    // 检查目标收藏夹是否存在
    if (!result.data.collections.some(c => c.id === toCollectionId)) {
      return createError(FavoritesErrorCodes.COLLECTION_NOT_FOUND, '目标收藏夹不存在')
    }

    // 查找原收藏项
    const favorite = result.data.favorites.find(
    ...
    // 检查目标收藏夹是否已有该壁纸
    if (result.data.favorites.some(f => f.wallpaperId === wallpaperId && f.collectionId === toCollectionId)) {
...
<acceptance_criteria>
- File `src/repositories/favorites.repository.ts` exists
- File exports `favoritesRepository` object with all methods
- `getCollections()` method exists returning `IpcResponse<Collection[]>`
- `createCollection(name: string)` method exists returning `IpcResponse<Collection>`
- `renameCollection(id: string, name: string)` method exists
- `deleteCollection(id: string)` method exists with isDefault check
- `getFavorites(collectionId?: string)` method exists
- `addFavorite(item: FavoriteItem)` method exists
- `removeFavorite(wallpaperId: string, collectionId: string)` method exists
- `moveFavorite(wallpaperId, fromCollectionId, toCollectionId)` method exists
- `isFavorite(wallpaperId: string)` method exists
- `getCollectionsForWallpaper(wallpaperId: string)` method exists
- Default collection initialization on first `getData()` call
- Error codes match FavoritesErrorCodes from types
</acceptance_criteria>

---

### Task 2: Export Repository from Index

<read_first>
- /Volumes/DATA/Code/Vscode/wallhaven/src/repositories/index.ts — existing export pattern
</read_first>

<action>
Add the following export to `src/repositories/index.ts`:
...
<acceptance_criteria>
- `src/repositories/index.ts` contains `export { favoritesRepository } from './favorites.repository'`
- Import `import { favoritesRepository } from '@/repositories'` works
</acceptance_criteria>
...
2. Verify repository methods exist:
   ```bash
   grep -E "async (getCollections|createCollection|deleteCollection|addFavorite|removeFavorite|moveFavorite|isFavorite|getCollectionsForWallpaper)" src/repositories/favorites.repository.ts
   ```
   Expected: All 8 methods found

3. Test default collection creation logic:
   - First call to `getData()` with empty storage should create default collection
   - `deleteCollection()` on default collection should return error with code `COLLECTION_IS_DEFAULT`

## Success Criteria

- [ ] `src/repositories/favorites.repository.ts` created with all methods
- [ ] Repository exported from `src/repositories/index.ts`
- [ ] TypeScript compilation passes
- [ ] All methods return `IpcResponse<T>` format
- [ ] Default collection initialized on first access
- [ ] Default collection cannot be deleted (COLL-04)
